...
const int trapdoorOpenPosition = 20;
const int trapdoorClosedPosition = 128;

bool trapdoorIsOpen = false, enterWasPressed = true;

task main()
{
	servo[sTrapdoor] = trapdoorClosedPosition;
	...
	while (true) {
		...
		if (nNxtButtonPressed == kLeftButton) {
			...
			if (!enterWasPressed) {
				trapdoorIsOpen = !trapdoorIsOpen;
				servo[sTrapdoor] = trapdoorIsOpen ? trapdoorOpenPosition : trapdoorClosedPosition;
			}
		}

		motor[mIntake] = 75 * spin;

		enterWasPressed = nNxtButtonPressed == kEnterButton;
	}
}
